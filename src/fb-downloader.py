...
# Static output directory for Facebook
OUTPUT_DIR = r"F:\FB_VIDEOS"
...
def download_video_worker():
    """Background worker for downloading Facebook videos"""
    while True:
        url = video_queue.get()
        if url is None:
            break
        try:
            log_message(f"📥 Descargando de Facebook: {url}")
            ...
            with yt_dlp.YoutubeDL(info_opts) as ydl:
                ...
                if not video_title or video_title == "Facebook video":
                    video_title = info.get("description", "facebook_video")[:50]
            ...
            # Download Facebook video (best quality with audio)
            ydl_opts = {
                "format": "best",
...
def download_video():
    ...
    url = url_entry.get()
    if not url:
        log_message("❌ Error: Por favor, ingrese un enlace de Facebook.")
        return
    video_queue.put(url)
...
# GUI Configuration
root = tk.Tk()
root.title("Descargador de Facebook")

# Label
label = tk.Label(root, text="Ingrese el enlace de Facebook:")
label.pack(pady=10)
