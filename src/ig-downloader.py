#!/bin/env python3
# IG Downloader - Instagram Reels & Stories
...
# Static output directory for IG
OUTPUT_DIR = r"F:\IG_VIDEOS"
...
def download_video_worker():
    """Background worker for downloading Instagram videos"""
    while True:
        url = video_queue.get()
        if url is None:
            break
        try:
            log_message(f"📥 Descargando de Instagram: {url}")
            ...
            with yt_dlp.YoutubeDL(info_opts) as ydl:
                ...
                if not video_title or video_title == "Instagram video":
                    video_title = info.get("description", "instagram_video")[:50]
            ...
            # Download Instagram video (best quality with audio)
            ydl_opts = {
                "format": "best",
...
def download_video():
    ...
    url = url_entry.get()
    if not url:
        log_message("❌ Error: Por favor, ingrese un enlace de Instagram.")
        return
    video_queue.put(url)
...
# GUI Configuration
root = tk.Tk()
root.title("Descargador de Instagram (Reels & Stories)")

# Label
label = tk.Label(root, text="Ingrese el enlace de Instagram (Reel o Story):")
label.pack(pady=10)
